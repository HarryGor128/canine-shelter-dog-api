...
const newMessage: ChatMessage = {
    msg: 'add new text',
    createTime: unixTimeSeconds,
    email: 'freeac@test.com',
    id: 0,
    type: 'text',
    lastUpdate: unixTimeSeconds,
};
...
const testCase = (koa: Koa) => {
    ...
    describe('Add dog testing', () => {
        test('Upload dog photo and Add new dog info', async () => {
            const uploadResult = await request(koa.callback())
                .post('/dog/uploadDogPhoto')
                .send(uploadFile);
            expect(uploadResult.statusCode).toEqual(201);

            const result = await request(koa.callback())
                .post('/dog/addNewDogInfo')
                .send({ ...newDog, photo: (await uploadResult.request).text });
            expect(result.statusCode).toEqual(201);
        });
    });

    describe('Delete dog testing', () => {
    ...
    describe('Add dog to favorites list and delete', () => {
    ...
    describe('Delete dog to favorites list', () => {
    ...
    describe('Add message to chat room', () => {
        test('Add a message to chat room', async () => {
            const result = await request(koa.callback())
    ...
    describe('Delete message from chat room', () => {
