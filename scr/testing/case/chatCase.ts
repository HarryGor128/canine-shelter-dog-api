...
const newMessage: ChatMessage = {
    msg: 'add new text',
    createTime: 1715239312,
    email: 'freeac@test.com',
    id: 0,
    type: 'text',
    lastUpdate: 1715239312,
};
...
const chatCase = (koa: Koa) => {
    describe('Chat endpoint testing', () => {
        test('Login', async () => {
        ...
        test('Get all chat history', async () => {
        ...
        test('Update chat message', async () => {
            const result = await request(koa.callback())
                .put('/chat/updateMessage')
                .send(updateMessage);
            expect(result.statusCode).toEqual(200);
        });
        ...
        test('Upload a chat message photo', async () => {
            const result = await request(koa.callback())
